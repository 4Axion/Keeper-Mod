...
class PlayState extends MusicBeatState
{
	...
	public static var songEnd:Bool = false;
	...
	public var diaPosz:Array<String> = ["0,0"];
	public static var diaPos:String;
	private var playLoopF:Bool = true;
	...
	//public static var diaPos:Array<String>;
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'senpai':
				dialogue = CoolUtil.coolTextFile(Paths.txt('DIALOGUE/senpaiDialogue'));
				diaPosz = CoolUtil.coolTextFile(Paths.txt('DIALOGUE/'+"Senpai"+"Pos"));
				diaPos = diaPosz[0];
			case 'roses':
				dialogue = CoolUtil.coolTextFile(Paths.txt('DIALOGUE/rosesDialogue'));
				diaPosz = CoolUtil.coolTextFile(Paths.txt('DIALOGUE/'+"Senpai"+"Pos"));
				diaPos = diaPosz[0];
			case 'thorns':
				dialogue = CoolUtil.coolTextFile(Paths.txt('DIALOGUE/thornsDialogue'));
				diaPosz = CoolUtil.coolTextFile(Paths.txt('DIALOGUE/'+"Senpai"+"Pos"));
				diaPos = diaPosz[0];
			case 'pet':
				dialogue = CoolUtil.coolTextFile(Paths.txt('DIALOGUE/petDialogue'));
				diaPosz = CoolUtil.coolTextFile(Paths.txt('DIALOGUE/'+"keeper"+"Pos"));
				diaPos = diaPosz[0];
			case 'lost':
				dialogue = CoolUtil.coolTextFile(Paths.txt('DIALOGUE/lostDialogue'));
				diaPosz = CoolUtil.coolTextFile(Paths.txt('DIALOGUE/'+"keeper"+"Pos"));
				diaPos = diaPosz[0];
			case 'isolation':
				dialogue = CoolUtil.coolTextFile(Paths.txt('DIALOGUE/isolationDialogue'));
				diaPosz = CoolUtil.coolTextFile(Paths.txt('DIALOGUE/'+"keeper"+"Pos"));
				diaPos = diaPosz[0];
			case 'keeper':
				dialogue = CoolUtil.coolTextFile(Paths.txt('DIALOGUE/keeperDialogue'));
				diaPosz = CoolUtil.coolTextFile(Paths.txt('DIALOGUE/'+"keeper"+"Pos"));
				diaPos = diaPosz[0];
		}
				...
				case 'green':
					{
							defaultCamZoom = 0.9;
							curStage = 'stage';
							var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('Slimeback'));
							bg.antialiasing = true;
							bg.scrollFactor.set(0.9, 0.9);
							bg.active = false;
							add(bg);
		
							var stageFront:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image('Slimefront'));
							stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
							stageFront.updateHitbox();
							stageFront.antialiasing = true;
							stageFront.scrollFactor.set(0.9, 0.9);
							...
							var stageCurtains:FlxSprite = new FlxSprite(-500, -300).loadGraphic(Paths.image('Slimecurtains'));
							stageCurtains.setGraphicSize(Std.int(stageCurtains.width * 0.9));
							stageCurtains.updateHitbox();
							stageCurtains.antialiasing = true;
							stageCurtains.scrollFactor.set(1.3, 1.3);
	...
	{
		if(FlxG.keys.justPressed.PAGEDOWN || songEnd)
			{
				songEnd = false;
				endSong();
			}
	...
	function endSong():Void
	{
		...
		else
		{
			if (isStoryMode)
			{
				...
				if(storyWeek == 7 && playLoopF && SONG.song == "Pet")
					{
						storyPlaylist.insert(3,"Keeper");
						playLoopF = false;
					}
